...
/*   Updated: 2022/10/27 21:51:25 by jaka          ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void    ShrubberyCreationForm::execute(const Bureaucrat& bur)
{
    ...
    try
    {
        ...
        if (bur.getGrade() > this->getReqGradeExec())
            throw GradeTooLowException("Catch shrubbery: Can't execute, the bur has too low grade.");
        else
        {
...
{
    std::cout << GRN"   Drawing shrubbery to file " << _target << "\n" RES;
    
    /////////////////////////////////
    // MAYBE NOT NEEDED, JUST DO "file_shrubbery"
