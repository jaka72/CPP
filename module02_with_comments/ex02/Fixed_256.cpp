...
Fixed::Fixed(const int i)
{
	...
	// _fpn = i * 256; // same as i * 256
	// _fpn = i * 1000; // same as i * 256
...
{
	// std::cout << "a) this++:" << this->_fpn << '\n';
	++this->_fpn;
	// std::cout << "b) this++:" << this->_fpn << '\n';
	return (*this);
}
...
{
	Fixed temp (*this);
	// std::cout << "c) ++this: " << this->_fpn << ".  tmp: " << temp._fpn << '\n';
	++this->_fpn;
	// std::cout << "d) ++this: " << this->_fpn << ".  tmp: " << temp._fpn << '\n';
	return (temp);
}
...
//	IT WAS SHIFTED << _frac_bits ( * 256) IN THE CONASTRUCTOR(int) 
Fixed Fixed::operator* (const Fixed &fixed)
{
	Fixed temp;
	temp._fpn = (_fpn * fixed._fpn) >> _frac_bits;
	// temp._fpn = (_fpn * fixed._fpn) / 256;
	// temp._fpn = (_fpn * fixed._fpn) / 1000;
	return temp;
}
...
float Fixed::toFloat(void) const
{
	...
	// a = _fpn / (float)256; // MUST BE CASTED TO FLOAT !!!
	// a = _fpn / (float)1000; // MUST BE CASTED TO FLOAT !!!
	return (a);
}
