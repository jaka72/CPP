...
/*   Updated: 2022/11/18 17:18:44 by jmurovec      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
{
	//std::cout << "Convert: \n";
	//std::cout << "From convert():  _type: " << _type << "\n";
	// std::cout << "From convert():  _f: " << _f << "\n";
	// std::cout << "From convert():  _d: " << _d << "\n";
	...
	else if (_type == FLOAT)
	{
		_c 		= (char)_f;
		_intg 	= (int)_f;
		//	_d 	= (double)_f;  // maybe this is problematic form my error
		if (isinf(_f))		// SOMETHING NOT GOOD, SHOULD BE inf == true ???
		{
			//_d 	= (double)_f;  // it caused _d turning into inf, when _f was inf
			
			//std::cout << "From convert(): (type float)  _f: " << _f << "\n";
			//std::cout << "From convert(): (type float)  _d: " << _d << "\n";
		}
	}
...
int	Conversion::storeCorrectType()
{
	if (checkIntOverflow(&_inputStr[_start], INT_MAX) == 1)
		_int_overflow = 1;
	if (_type == INVALID_INPUT) // WHERE WAS THIS SET SO FAR ???
...
int	Conversion::processInputString()
{
	...
	if (ret != 0)
		return (ret); // MAYBE HERE RETURN INVALID_INPUT
